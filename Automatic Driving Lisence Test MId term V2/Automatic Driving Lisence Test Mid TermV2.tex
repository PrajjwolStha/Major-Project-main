...
\makebox[8cm]{Nitesh Poudel \hfill [41052]}\\
\makebox[8cm]{Pawan Bhandari \hfill [41053]}\\
\makebox[8cm]{Prajjwol Shrestha \hfill [41054]}\\
\makebox[8cm]{Ujjwal Naral \hfill [41057]}
...
\newcommand{\defDay}{27} %specify Defense Day- 1, 2, ...

\newcommand{\supervisor}{Sachin Shrestha} % Specify Name of Supervisor for Major Project (write "none" if no Supervisor is assigned)
\newcommand{\degSup}{Nepal Engineering College)} %Specify Designation of Supervisor for Major Project, use multiple lines (\\) if necessary
...
\emph{Driving test automation,object detection, image processing}
...
\end{itemize}
According to the NADA Automobiles Association of Nepal, the number of purchases of two-wheelers has more than doubled since the COVID-19 pandemic hit the country\cite{fivethousandapplicants}. With the increase in purchase of two-wheelers the number of applicants applying for driving license has also increased. According to the DoTM, more than 300,000 online service seekers in the Kathmandu Valley alone have not been able to participate in the trial exams yet. More than 700,000 service seekers have been waiting for the trials by filling online forms during the period of January to April\cite{fivethousandapplicants}\cite{twoyearwait}. However, the date for examinations till January 2022 has been booked as the Transportation Management Office has not been able to provide services as per the pressure of the service recipients. Of many factors that contribute to the TMO's inability to provide services as per requirement one is the inefficiency in the traditional driving license exam system. 
...
the functioning processes of the human brain \cite{nervousactivity}. They
...
the optimal solution for these problems \cite{introtoann}.
...
function is that it provides a smooth transition as input values change, i.e., a small change in input produces a small change in output.
...
organization of animal visual cortex \cite{hubel:monkey} \cite{fukushima:neocognitronbc} and designed to
...
\begin{center}
	%\includegraphics[width = 3in]{images/logo.png}
	\includegraphics[width = 1.3in]{images/system flowchartdrawio.png}
...
The data set used for training, testing, and validation consisted of a total of 4,686 images. This includes 4,374 images used for training, 204 images used for testing, and 108 images used for validation. The data set  consists of images captured from a scaled-down version of a car, and simulations created using a game engine. The data set includes both successful and unsuccessful attempts at completing a driving test. The images were captured using a high-resolution camera mounted directly above the driving track giving top down view of the overall track. The images were then preprocessed to remove any noise or distortions caused by the camera. The simulations were created using a game engine and were designed to mimic real-world driving scenarios.
\subsection{Data Collection Process}
The images were captured during a driving test that was conducted using the scaled-down version of the car. The driving test was designed to mimic real-world driving exam scenario.. The simulations were created using a game engine, which allowed for the creation of a wide range of driving scenarios, including different weather conditions and traffic patterns.
\subsection{Data Cleaning and Preprocessing}
Several cleaning and preprocessing steps were taken to ensure the quality and reliability of the data set. These steps included removing any images with low quality or distortion caused by the camera, and standardizing the image size and format. Outliers were also identified and removed from the data set to ensure the accuracy of the results.
\section{Testing}
...
\section{Conclusion}
The project “Automated Driving License Test” has been a great learning opportunity for us in terms of hardware and software interfacing. The main goal of our project was to make automation cost effective and user friendly. We achieved this by using easily and cheaply available components as well as open-source software.
\\
Since automation is the future of humanity, it must be accessible to everyone regardless of their economic status or expertise. The students of engineering in the later years might be able to learn something from this project and expand upon it. This project is only a tiny part of a robotic
system but an essential one.
...
\begin{center}
	%\includegraphics[width = 3in]{images/logo.png}
	\includegraphics[width = 6.5in]{images/snapshot.jpg}
	%\caption{Flow chart of System} %figure name
	\label{figObjectDetectionusingYOLO} % for referencing
\end{center}
\end{figure}
